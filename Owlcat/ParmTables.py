...
class _AxisStats(object):
    ...
    def update(self):
        if self.cells:
            # axis range
            self.minmax = min([x0 - dx / 2 for x0, dx in self.cells.iteritems()]), max(
                [x0 + dx / 2 for x0, dx in self.cells.iteritems()])
...
class FunkSet(object):
    ...
    def apply(self, op_func, slicing=[], outtab=None, remove=False):
        ...
        dprintf(3, "%s: %s() transformed %d input funklets over %d slices into %d output funklets\n", self.name,
                op_func.__name__, num_infunk, num_slices, num_outfunk)
