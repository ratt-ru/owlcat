...
def make_diffgain_plots (filename, prefix=None, ylim=None, subset=None, ant=None, feed_type="rl"):
  ...
  print "making diffgain plots for",srcnames;
...
def make_ifrgain_plots (filename,
  ...
  if msname:
    try:
      ...
      traceback.print_exc();
      print("WARNING:: can't access %s antenna table, so IFR gain versus baseline length plots will not be made"%maname);
  else:
    print("WARNING:: MS not specified, thus no antenna info. IFR gain versus baseline length plots will not be made");
